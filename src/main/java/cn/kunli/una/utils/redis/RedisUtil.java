...
public class RedisUtil<T> {
     ...
     * 模糊查询key是否存在
     *
     * @param partten 模糊key（带通配符）
     * @return true 存在 false不存在
     */
    public Set<String> hasKeys(String partten) {
        try {
            Set<String> keys = redisTemplate.keys(partten);
            return keys;
        } catch (Exception e) {
            //如果redis不可用，标记
            if (e.getMessage().indexOf("Cannot get Jedis connection") != -1) setIsConnect(false);
    ...
    public boolean set(String key, T value) {
        try {
            ...
            //如果redis不可用，标记
            if (e.getMessage().indexOf("Cannot get Jedis connection") != -1) {
                setIsConnect(false);
            }
            e.printStackTrace();
            return false;
