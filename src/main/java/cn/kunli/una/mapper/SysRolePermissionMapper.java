...
public interface SysRolePermissionMapper extends BasicMapper<SysRolePermission> {
    ...
    @Insert("insert into sys_role_permission (role_id,permission_id,scope_dcode) select #{roleId},id,'permission_scope_none' from sys_permission where is_delete=0 order by id")
    Integer insertByRoleId(@Param("roleId") Integer roleId);

    @Insert("insert into sys_role_permission (role_id,permission_id,scope_dcode) select id,#{permissionId},'permission_scope_none' from sys_role where is_delete=0 order by id")
